...
async function handleQuickView(modalPath, closeButtonHtml) {
  ...
  await openModal(
    modalPath,
    true, // is modal
    'line-info-inner-page', // dialog ID
    ['cmp-line-info-inner-page'], // classes
    null,
    closeButtonHtml,
  );
}
...
export default async function decorate(block) {
  try {
    const config = await getBlockConfigs(block, DEFAULT_CONFIG, 'line-info');
    ...
    if (containerBgColorDefault) {
      if (containerBgColorDefault2) {
        // 渐变 - 两个颜色都配置时
        inlineStyle += `--line-info-bg-default-image: linear-gradient(270deg, ${containerBgColorDefault} 0%, ${containerBgColorDefault2} 100%);`;
      } else {
        // 纯色 - 只配置一个颜色
        inlineStyle += `--line-info-bg-default: ${containerBgColorDefault};`;
        inlineStyle += '--line-info-bg-default-image: none;';
      }
    }
    // Hover 背景色 CSS 变量
    if (containerBgColorHover) {
      if (containerBgColorHover2) {
        // 渐变 - 两个颜色都配置时
        inlineStyle += `--line-info-bg-hover-image: linear-gradient(270deg, ${containerBgColorHover} 0%, ${containerBgColorHover2} 100%);`;
      } else {
        // 纯色 - 只配置一个颜色
        inlineStyle += `--line-info-bg-hover: ${containerBgColorHover};`;
        inlineStyle += '--line-info-bg-hover-image: none;';
      }
    }
    // Active 背景色 CSS 变量
    if (containerBgColorActive) {
      if (containerBgColorActive2) {
        // 渐变 - 两个颜色都配置时
        inlineStyle += `--line-info-bg-active-image: linear-gradient(270deg, ${containerBgColorActive} 0%, ${containerBgColorActive2} 100%);`;
      } else {
        // 纯色 - 只配置一个颜色
        inlineStyle += `--line-info-bg-active: ${containerBgColorActive};`;
        inlineStyle += '--line-info-bg-active-image: none;';
      }
    }
    // 字体颜色 CSS 变量
    if (fontColorDefault) {
      inlineStyle += `--line-info-color-default: ${fontColorDefault};`;
    }
    if (fontColorHover) {
      inlineStyle += `--line-info-color-hover: ${fontColorHover};`;
    }
    if (fontColorActive) {
      inlineStyle += `--line-info-color-active: ${fontColorActive};`;
    }
    // 圆角自定义
    ...
    if (supportsIcon(style)) {
      ...
      if (iconStyle === 'svg' || iconStyle === '') {
        ...
        iconHtml = `
          <div class="line-info-icon-wrapper flex items-center justify-center transition-all"
               style="${iconBgStyle}"
               data-bg-default="${iconBgColorDefault}"
               data-bg-hover="${iconBgColorHover}"
               data-bg-active="${iconBgColorActive}">
            <svg viewBox="0 0 24 24" class="line-info-icon-svg">
              <defs>
        ...
        iconHtml = `
          <div class="line-info-icon-wrapper line-info-icon-png flex items-center justify-center transition-all" style="${iconBgStyle}">
            <img src="${iconAssetDefault}" alt="icon" class="line-info-icon-default object-cover"/>
            <img src="${iconAssetHover}" alt="icon hover" class="line-info-icon-hover object-cover hidden"/>
            <img src="${iconAssetActive}" alt="icon active" class="line-info-icon-active object-cover hidden"/>
          </div>
        `;
      }
    }
    ...
    const buttonHtml = `
      <a href="${href}"
         class="line-info-component inline-flex items-center justify-center transition-all duration-300 cursor-pointer ${fontDesktop} ${fontMobile}"
         data-link-type="${linkType}"
         data-is-external="${isExternal}"
         data-style="${style}"
         ${inlineStyle ? `style="${inlineStyle.trim()}"` : ''}
         data-filled="${filled}">
        <span class="line-info-label">${label}</span>
        ${iconHtml}
      </a>
    `;

    // 移动 AEM 编辑器 instrumentation 属性到按钮元素（必须在修改 innerHTML 之前）
    const oldLineInfoElement = block.querySelector('.line-info-component');
    if (oldLineInfoElement) {
      moveInstrumentation(block, oldLineInfoElement);
    }

    block.innerHTML = buttonHtml;

    const lineInfoElement = block.querySelector('.line-info-component');
    if (lineInfoElement) {
      // 外部链接处理
      if (isExternal && href) {
        lineInfoElement.setAttribute('target', '_blank');
        lineInfoElement.setAttribute('rel', 'noopener noreferrer');
      }
      // Inner Page 处理 - 点击时调用 handleQuickView
      if (!isExternal && linkType === 'inner' && innerPageLink) {
        lineInfoElement.addEventListener('click', (e) => {
          e.preventDefault();
          // 构建关闭按钮 HTML
